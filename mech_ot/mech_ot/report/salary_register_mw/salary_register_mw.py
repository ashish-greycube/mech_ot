...
def execute(filters=None):
	...
	emp_map = get_employee_details_map()
	...
	for ss in salary_slips:
		row = {
			...
			"date_of_joining": emp_map.get(ss.employee).get("date_of_joining"),
			"branch": ss.branch,
			"department": ss.department,
			"designation": ss.designation,
			"category": emp_map.get(ss.employee).get("category"),
			"company": ss.company,
...
def get_salary_slips(filters, company_currency):
	...
	query = frappe.qb.from_(salary_slip).left_join(employee).on(salary_slip.employee == employee.name).select(salary_slip.star)
	...
	if filters.get("category"):
		query = query.where(employee.custom_category == filters["category"])
...
def get_employee_details_map():
	employee = frappe.qb.DocType("Employee")

	result = (frappe.qb.from_(employee).select(employee.name, employee.date_of_joining, employee.custom_category)).run(as_dict=1)
	# print("Result : ",result)
	emp_map = {}
	for d in result:
		emp_map[d.name]={
			"date_of_joining": d.date_of_joining,
			"category": d.custom_category
		}
	# print("Dict:",emp_map)
	return emp_map
